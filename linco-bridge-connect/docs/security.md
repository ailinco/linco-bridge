...
Linco Bridge connector 运行在用户本机，负责连接远端 IM 和本地 Agent。安全边界的核心原则是：凭证留在本机，文件读取受限，危险操作需要明确策略。
...
`/get <路径>` 只应返回当前工作目录、`/project` 当前列出的项目、会话运行目录或附件目录内的普通非隐藏文件，并且必须通过大小、类型和路径校验。相对路径只在当前会话目录中解析；绝对路径在读取前按真实路径校验归属，项目内的软链接不能跳到这些允许目录之外。不要放宽到任意绝对路径。
